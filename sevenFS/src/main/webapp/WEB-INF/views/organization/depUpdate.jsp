  ...
  <%@ include file="../layout/header.jsp" %>
	<section class="section">
		<div class="container-fluid">
			<form id="frm" action="/depUpdatePost" method="post" id="depUpdateForm" class="needs-validation" novalidate>
				<c:set var="deptDetail" value="${deptData.deptDetail}"></c:set>
				<input type="hidden" value="${deptDetail.cmmnCode}" name="cmmnCode" />
				<input type="hidden" value="${deptDetail.cmmnCodeSn}" name="cmmnCodeSn" />
 ...
 <script type="text/javascript">
  ...
  $(function(){
	$("#updateBtn").on("click", function(e){
        ...
        // 부서이름
        const cmmnCodeNm = $('#cmmnCodeNm').val();
        // 부서 설명
        const cmmnCodeDc = $('#cmmnCodeDc').val();
		
        if(cmmnCodeNm || cmmnCodeDc){
 			swal({
			  text: '수정되었습니다.',
			  icon: 'success'
			})
